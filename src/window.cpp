...
Window::Window(PluginInstance &p) :
    ...
    styles([&]{
        ...
        for (const auto &path : styleDirectories())
            for (const auto files = QDir(path).entryInfoList({u"*.ini"_s},
                                                             QDir::Files | QDir::NoSymLinks);
                 const auto &file : files)
                t.emplace(file.baseName(), file.canonicalFilePath().toStdString());
        return t;
    }()),
...
void Window::initializeUi()
{
    ...
    QTimer::singleShot(0, this, [this]{
        try {
            ...
            WARN << "Failed to apply style. Reverting to default style.";
            applyStyle(Style::fromApplicationPalette());
        }
    });
...
void Window::onStyleFileChanged()
{
    try {
        ...
        applyStyle(Style::fromStyleFile(styles.at(dark_mode ? style_dark_ : style_light_)));
...
void Window::applyStyle(const QString& name)
{
    ...
    {
        // Set watch before application, because applyStyle might fail but the watch should remain
        const auto &path = styles.at(name);
        style_watcher.addPath(toQString(path));
        applyStyle(Style::fromStyleFile(path));
    }
}
...
void Window::applyStyle(const Style &style)
{
    ...
    // this (Frame)
    this->setBackgroundBrush(style.window.background_brush);
    this->setBorderBrush(style.window.border_brush);
    this->setBorderWidth(style.window.border_width);
    this->setBorderRadius(style.window.border_radius);
    input_frame->setFixedWidth(style.window.width); // set on frame because it defines frame size

    // this (WindowFrame)

    this->setShadowSize(style.window.shadow_size);
    this->setShadowOffset(style.window.shadow_offset);
    this->setShadowBrush(style.window.shadow_brush);

    // this (layout)
    layout()->setContentsMargins(style.window.padding, style.window.padding,
                                 style.window.padding, style.window.padding);
    layout()->setSpacing(style.window.spacing);


    // input_frame (Frame)
    input_frame->setBackgroundBrush(style.input.background_brush);
    input_frame->setBorderBrush(style.input.border_brush);
    input_frame->setBorderWidth(style.input.border_width);
    input_frame->setBorderRadius(style.input.border_radius);

    // input_line (InputLine)
    auto f = input_line->font();
    f.setPointSize(style.input.font_size);
    input_line->setFont(f);
    input_line->setTriggerColor(style.input.trigger_color);
    input_line->setInputActionColor(style.input.action_color);
    input_line->setInputHintColor(style.input.hint_color);
    input_frame->setContentsMargins(style.input.padding, style.input.padding,
                                    style.input.padding, style.input.padding);

    // settings_button (SettingsButton) // TODO: oh hell this is weird. redesign
    settings_button_color_ = style.settings_button.color; // ??
    settings_button->color = style.settings_button.color;
    settings_button->color.setAlpha(0);
    settings_button_color_highlight_ = style.settings_button.highlight_color;

    const auto fm = input_line->fontMetrics();
    const auto line_height = fm.lineSpacing();
    const auto button_size = fm.ascent();
    auto button_margins = (line_height - button_size)/2;
    settings_button->setFixedSize(line_height, line_height);
    settings_button->setContentsMargins(button_margins, button_margins,
                                        button_margins, button_margins);

    results_list->setIconSize(style.results.icon_size);
    results_list->setTextFontSize(style.results.text_font_size);
    results_list->setSubtextFontSize(style.results.subtext_font_size);
    results_list->setHorizontalSpacing(style.results.horizontal_space);
    results_list->setVerticalSpacing(style.results.vertical_space);
    results_list->setPadding(style.results.padding);
    results_list->setTextColor(style.results.text_color);
    results_list->setSubtextColor(style.results.subtext_color);
    results_list->setSelectionTextColor(style.results.selection.text_color);
    results_list->setSelectionSubtextColor(style.results.selection.subtext_color);
    results_list->setSelectionBackgroundBrush(style.results.selection.background_brush);
    results_list->setSelectionBorderBrush(style.results.selection.border_brush);
    results_list->setSelectionBorderRadius(style.results.selection.border_radius);
    results_list->setSelectionBorderWidth(style.results.selection.border_width);


    actions_list->setTextFontSize(style.actions.font_size);
    actions_list->setPadding(style.actions.padding);
    actions_list->setTextColor(style.actions.text_color);
    actions_list->setSelectionTextColor(style.actions.selection.text_color);
    actions_list->setSelectionBackgroundBrush(style.actions.selection.background_brush);
    actions_list->setSelectionBorderBrush(style.actions.selection.border_brush);
    actions_list->setSelectionBorderRadius(style.actions.selection.border_radius);
    actions_list->setSelectionBorderWidth(style.actions.selection.border_width);


    if (auto m = results_list->model(); m) // select first after reset
        if (m->rowCount() > 0)
...
void Window::setStyleLight(const QString &name)
{
    ...
    if (!dark_mode)
        try {
            ...
            WARN << "Failed to apply style:" << e.what();
        }
